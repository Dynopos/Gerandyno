    ...
    @if ($emailRoute)
        <form method="POST" action="{{ route($emailRoute) }}" x-data="{ sending: false }" @submit="sending = true">
            ...
            <button
                type="submit"
                :disabled="sending"
                class="inline-flex items-center gap-1.5 rounded-lg bg-violet-600 px-3 py-1.5 text-sm font-medium text-white shadow-sm transition hover:bg-violet-700 disabled:opacity-60"
            >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4">
